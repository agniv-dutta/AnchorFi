      ...
      {rows.map(([label, cat], idx) => (
        <div key={label} style={{ marginBottom: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "110px 1fr 28px", gap: 8, alignItems: "center" }}>
            <div className="k-mono" style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: "#999", width: 110 }}>{label}</div>
            ...
            <div className="k-mono" style={{ fontSize: 14, fontWeight: 900, textAlign: "right" }}>{cat.score}</div>
          </div>
          <div style={{ marginTop: 2 }}>
            {(cat.flags || []).map((flag) => (
              <span key={`${label}-${flag}`} style={{ border: `1px solid ${cat.score >= 70 ? "#c0392b" : "#111"}`, color: cat.score >= 70 ? "#c0392b" : "#111", fontFamily: "'Courier New', monospace", fontSize: 8, padding: "2px 6px", display: "inline-block", margin: 2, textTransform: "uppercase", letterSpacing: "1px" }}>{flag}</span>
            ))}
          </div>
        </div>
      ))}
